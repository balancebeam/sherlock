 ...
 */
 package io.pddl.router.database.execution.support;
...
import io.pddl.router.database.execution.RequestDepository;

public class DefaultConcurrentRequestProcessor implements ConcurrentRequestProcessor {
    ...
    public List<Object> process(List<ConcurrentRequest> requests) {
        ...
        try {
        	final ExecutorContext mainCtx= ExecutorContextHolder.getContext();
            for (RequestDepository rdepo : requestsDepo) {
                ...
                futures.add(request.getExecutor().submit(new Callable<Object>() {
                    public Object call() throws Exception {
                        try {
                        	ExecutorContext context= ((ExecutorContextSupport)mainCtx).clone();
                    		((ExecutorContextSupport)context).setPartitionDataSource(request.getPartitionDataSource());
