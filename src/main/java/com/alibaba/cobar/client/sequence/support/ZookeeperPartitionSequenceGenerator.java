...
public class ZookeeperPartitionSequenceGenerator implements SequenceGenerator, InitializingBean {
	...
	class DBSequenceBuilderTask implements Callable<AtomicLong> {
		...
		private AtomicLong getStartIndex() throws Exception{
			...
			client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath("/seq/"+name, String.valueOf(boundaryMaxValue).getBytes(Charset.forName("UTF-8")));
			...
			while(true){
				...
				if(null != client.checkExists().forPath("/seq/"+name)){//已经存在
					...
					if(null == client.checkExists().forPath("/seq/"+name)){
						...
						client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath("/seq/"+name, data.getBytes(Charset.forName("UTF-8")));
