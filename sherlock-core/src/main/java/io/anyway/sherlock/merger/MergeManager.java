...
//					}
	...
	private void mergeAggre2Row(List<AggregationColumn> aggregations,Row mergeRow,Map<Integer,Object> aggreHash){
		for(AggregationColumn c: aggregations){
			...
			if(c.getAggregationType()== AggregationColumn.AggregationType.AVG){
	...
	private Map<AggregationColumn.AggregationType,AggreMerger> aggreMergerRepository= new HashMap<AggregationColumn.AggregationType,AggreMerger>();
	
	{
		//总数处理
		aggreMergerRepository.put(AggregationColumn.AggregationType.COUNT, new AggreMerger(){
			@Override
		...
		//总和处理
		aggreMergerRepository.put(AggregationColumn.AggregationType.SUM, new AggreMerger(){
			@Override
			public void merge(ResultSet rs, Map<Integer, Object> aggreHash, int columnIndex) throws SQLException {
		...
		//最大值
		aggreMergerRepository.put(AggregationColumn.AggregationType.MAX, new AggreMerger(){
			@Override
			public void merge(ResultSet rs, Map<Integer, Object> aggreHash, int columnIndex) throws SQLException {
				double val= (Double)rs.getObject(columnIndex);
				if(aggreHash.containsKey(columnIndex)){
		...
		//最小值
		aggreMergerRepository.put(AggregationColumn.AggregationType.MIN, new AggreMerger(){
			@Override
			public void merge(ResultSet rs, Map<Integer, Object> aggreHash, int columnIndex) throws SQLException {
				double val= (Double)rs.getObject(columnIndex);
				if(aggreHash.containsKey(columnIndex)){
		...
		//平局值
		aggreMergerRepository.put(AggregationColumn.AggregationType.AVG, new AggreMerger(){
			@Override
			public void merge(ResultSet rs, Map<Integer, Object> aggreHash, int columnIndex) throws SQLException {
				double val= (Double)rs.getObject(columnIndex);
